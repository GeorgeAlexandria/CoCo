    ...
    internal class FSharpClassifierService
    {
            ...
            FSharpParseFileResults parseResults, FSharpCheckFileResults checkResults, SnapshotSpan span)
        {
            ...
            try
            {
                ...
                {
                    foreach (var moduleOrNamespace in implFile.Item.Item6)
                    {
        ...
        {
            var type = !moduleOrNamespace.isModule 
                ? _namespaceType 
                : _moduleType;

            if (type.IsNotNull())
            {
                AddIdents(moduleOrNamespace.id, type);
            }
        ...
        private void Visit(Ast.SynModuleDecl moduleDecl)
        {
            switch (moduleDecl)
            {
                ...
                case Ast.SynModuleDecl.NestedModule nestedModule:
                    AddIdents(nestedModule.Item1.Item4, _moduleType);
        ...
        private void Visit(Ast.SynMemberDefn memberDefn)
        {
            switch (memberDefn)
            {
                ...
                case Ast.SynMemberDefn.LetBindings letBindings:
                    foreach (var item in letBindings.bindings)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynMemberDefn.AutoProperty property:
                    AddIdent(property.id, _propertyType);
                    Visit(property.expr);
                    break;
                ...
                case Ast.SynMemberDefn.ImplicitCtor ctor:
                    foreach (var item in ctor.ctorArgs)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynMemberDefn.Interface @interface:
                    ...
                    if (@interface.interfaceMembers.IsSome())
                    {
                        foreach (var item in @interface.interfaceMembers.Value)
                        {
                            Visit(item);
                        }
                    }
        ...
        private void Visit(Ast.SynTypeDefnRepr typeDefnRepr)
        {
            switch (typeDefnRepr)
            {
                case Ast.SynTypeDefnRepr.ObjectModel objectModel:
                    ...
                    var members = objectModel.members;
        ...
        private void Visit(Ast.SynType type)
        {
            switch (type)
            {
                ...
                case Ast.SynType.Tuple tuple:
                    foreach (var (_, typeName) in tuple.Item1)
                    {
                        Visit(typeName);
                    }
                    break;
        ...
        {
            foreach (var item in componentInfo.Item4)
            {
            ...
            foreach (var item in componentInfo.attributes)
            {
                Visit(item);
            }
        }
        ...
        {
            Visit(exceptionDefn.exnRepr);
            foreach (var item in exceptionDefn.members)
            {
                Visit(item);
            }
        }
        ...
        private void Visit(Ast.SynTypeDefnSimpleRepr synTypeDefn)
        {
            switch (synTypeDefn)
            {
                ...
                case Ast.SynTypeDefnSimpleRepr.Enum @enum:
                    foreach (var item in @enum.cases)
                    {
                        AddIdent(item.id, _enumFieldType);
                    }
                    break;
                ...
                case Ast.SynTypeDefnSimpleRepr.General general:
                    ...
                    {
                        foreach (var item in general.Item7.Value)
                        {
                            Visit(item);
                        }
                    }
        ...
        {
            var type = _unionType;
            if (_cache.TryGetValue(unionCase.id.idRange, out var use) && use.Symbol is FSharpEntity entity &&
                entity.IsFSharpExceptionDeclaration)
            {
                type = _classType;
            }

            AddIdent(unionCase.id, type);
            Visit(unionCase.caseType);
        }
        ...
        private void Visit(Ast.SynField field)
        {
            ...
            if (field.id.IsSome())
            {
                AddIdent(field.id.Value, _fieldType);
            }
        ...
        {
            foreach (var item in valSig.attributes)
            {
                Visit(item);
            }

            Visit(valSig.SynInfo);

            if (_cache.TryGetValue(valSig.id.idRange, out var use))
            {
                ...
                else if (_context.Parent is Ast.SynMemberSig.Member memberSig)
                {
                    classification = use.Symbol switch
                    {
                        ...
                        FSharpParameter _ => valSig.SynType is Ast.SynType.Fun
                            ? memberSig.memberFlags.IsInstance
                                ? _methodType
                                : _staticMethodType
                            : _propertyType,
                ...
                {
                    AddIdent(valSig.id, classification);
                }
                else
                {
            ...
            if (valSig.expr.IsSome())
            {
                Visit(valSig.expr.Value);
            }
        }
        ...
        {
            foreach (var item in argInfo.attributes)
            {
                Visit(item);
            }

            if (argInfo.id.IsSome())
            {
                _context = _context.WithParams(_context.Params.Add(argInfo.id.Value.idText));
                AddIdent(argInfo.id.Value, _parameterType);
            }
        }

        private void Visit(Ast.SynBinding binding)
        {
            foreach (var item in binding.attributes)
            {
                Visit(item);
            }
        ...
        private void Visit(Ast.SynPat pattern)
        {
            ...
            switch (pattern)
            {
                case Ast.SynPat.Named nameSyntax:
                    ...
                    if (_cache.TryGetValue(nameSyntax.id.idRange, out var symbolUse))
                    {
                        switch (symbolUse.Symbol)
                        {
                            case FSharpMemberOrFunctionOrValue some:
                                ...
                                else
                                {
                                    ...
                                    if (classification.IsNotNull())
                                    {
                                        AddIdent(nameSyntax.id, classification);
                                        break;
                                    }
                ...
                case Ast.SynPat.Attrib attrib:
                    Visit(attrib.Item1);
                    foreach (var item in attrib.attributes)
                    {
                        Visit(item);
                    }
                    break;
        ...
        private void Visit(Ast.LongIdentWithDots identWithDots)
        {
            ...
            foreach (var item in idents)
            {
                ...
                {
                    var type = field.DeclaringEntity.IsEnum
                        ? _enumFieldType
                        : _fieldType;
        ...
        private void Visit(Ast.SynAttribute attribute)
        {
        ...
        private void Visit(Ast.SynTypeConstraint constraint)
        {
            switch (constraint)
            {
                ...
                case Ast.SynTypeConstraint.WhereTyparIsDelegate @delegate:
                    ...
                    foreach (var item in @delegate.delegateTypes)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynTypeConstraint.WhereTyparIsEnum @enum:
                    ...
                    foreach (var item in @enum.enumTypes)
                    {
                        Visit(item);
                    }
                    break;
        ...
        private void Visit(Ast.SynExpr expression)
        {
            switch (expression)
            {
                ...
                case Ast.SynExpr.DotSet dotSet:
                    Visit(dotSet.expr);
                    Visit(dotSet.dotId);
                    Visit(dotSet.exprValue);
                    break;

                case Ast.SynExpr.DotIndexedGet indexedGet:
                    Visit(indexedGet.expr);
                    foreach (var item in indexedGet.indexExprs)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynExpr.LetOrUseBang letBang:
                    Visit(letBang.pattern);
                    Visit(letBang.rhsExpr);
                    Visit(letBang.bodyExpr);
                    break;
                ...
                case Ast.SynExpr.ObjExpr objExpr:
                    ...
                    if (objExpr.argOpt.IsSome())
                    {
                        // TODO: what's about ident?
                        var (exp, _) = objExpr.argOpt.Value;
                        Visit(exp);
                    }
                ...
                case Ast.SynExpr.While @while:
                    Visit(@while.whileBody);
                    Visit(@while.doBody);
                    break;

                case Ast.SynExpr.For @for:
                    AddIdent(@for.id, _localBindingType);
                    Visit(@for.idBody);
                    Visit(@for.toBody);
                    Visit(@for.doBody);
                    break;
                ...
                case Ast.SynExpr.MatchLambda lambda:
                    foreach (var item in lambda.clauses)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynExpr.TryWith tryWith:
                    ...
                    foreach (var item in tryWith.Item3)
                    {
                        Visit(item);
                    }
                    break;
                ...
                case Ast.SynExpr.IfThenElse ifThenElse:
                    Visit(ifThenElse.exprGuard);
                    Visit(ifThenElse.exprThen);
                    if (ifThenElse.optionalExprElse.IsSome())
                    {
                        Visit(ifThenElse.optionalExprElse.Value);
                    }
                    break;
        ...
        {
            if (_cache.TryGetValue(synTypar.id.idRange, out var use))
            {
                if (use.Symbol is FSharpEntity entity && TryClassifyType(entity, out var type))
                {
                    AddIdent(synTypar.id, type);
                    return;
                }
                if (use.Symbol is FSharpGenericParameter)
                {
                    AddIdent(synTypar.id, _typeParameterType);
                    return;
                }
            }
        ...
        {
            Visit(clause.Item1);
            if (clause.Item2.IsSome())
            {
                Visit(clause.Item2.Value);
            }
            Visit(clause.Item3);
        }
        ...
        private bool IsParameterByUse(FSharpMemberOrFunctionOrValue some) =>
            !some.IsModuleValueOrMember && _context.Params.Contains(some.LogicalName);

        // TODO:
        private bool IsParameterByDefn(FSharpMemberOrFunctionOrValue some) =>
            !some.IsModuleValueOrMember && _context.Parent is Ast.SynConstructorArgs;
        ...
        private bool IsExtension(FSharpMemberOrFunctionOrValue some)
        {
            ...
            return HasExtensionAttribute(some.Attributes) && HasExtensionAttribute(some.EnclosingEntity.Attributes);
        }
